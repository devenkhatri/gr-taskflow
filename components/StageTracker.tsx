        ...
        {STAGES.map((stage, idx) => {
          ...
          return (
            <div key={stage.key} className="flex flex-col items-center z-10 bg-white px-2">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${isReached
              ...
              <span className={`mt-3 text-xs font-semibold whitespace-nowrap ${isReached ? 'text-indigo-600' : 'text-slate-500'
                }`}>
                {stage.label}
              </span>
              {isCurrent && (
