        ...
        <div className="flex gap-4 h-full px-2 py-2">
          {columns.map((column) => {
            ...
            return (
              <div
                key={column.id}
                className={`flex flex-col bg-slate-50 rounded-xl border border-slate-200 transition-all duration-300 ${isCollapsed ? 'w-16' : 'flex-1 min-w-[280px]'}`}
              >
                {/* Column Header */}
                ...
                {/* Column Content */}
                <div className={`bg-slate-100/60 rounded-2xl flex-1 flex flex-col gap-3 min-h-0 border border-slate-200/50 transition-all duration-300 ${isCollapsed ? 'items-center py-4 w-12' : 'p-3 overflow-y-auto'}`}>
                  {isCollapsed ? (
                      ...
                      {columnTasks.length > 0 ? columnTasks.map((task) => {
                        ...
                        return (
                          <div
                            ...
                            className={`bg-white p-4 rounded-xl shadow-sm border transition-all cursor-pointer group flex-shrink-0 ${isHighlighted
                              ? 'border-amber-400 ring-2 ring-amber-100 bg-amber-50 shadow-md transform -translate-y-0.5'
                              : 'border-slate-200 hover:shadow-lg hover:border-indigo-400 hover:-translate-y-0.5'
                              }`}
                          >
